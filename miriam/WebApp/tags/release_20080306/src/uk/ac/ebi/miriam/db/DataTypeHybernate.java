...
public class DataTypeHybernate extends DataType
{
	...
	{
		logger.debug("Begin of the addion of a new data-type...");
...
*/
	...
	public void retrieveData(String poolName, String index)
	{
		...
		rs = pool.request(pool.getStatement(), sql);
		ArrayList<String> deprecated = MiriamUtilities.ArrayConvert(rs);
        setDeprecatedURIs(deprecated);
		...
		for (int i=0; i<getDocumentationIDs().size(); ++i)
		{
		    ...
		    if ((result == null) || (result.length == 0))
		    {
		    	logger.warn("A Miriam URI (" + uri + ") correponding to a piece of documentation has no physical location");
		    }
		    else
		    {
			    for (int j=0; j<result.length; ++j)
