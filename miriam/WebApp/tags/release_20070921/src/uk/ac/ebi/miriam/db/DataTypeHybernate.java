...
/**
 * <p>
 * Class built above <code>DataType</code>, in order to add database connectivity features (storage, retrieving, ...) to the object.
 * 
 * <p>
 * <dl>
 * <dt><b>Copyright:</b></dt>
 ...
 * 
			sql = "INSERT INTO mir_datatype (datatype_id, name, pattern, definition, date_creation, date_modif) VALUES ('" + index + "', '" + getName() + "', '" + getRegexp() + "', '" + getDefinition() + "', NOW(), NOW())";
...
*/
			PreparedStatement stmt01 = pool.getPreparedStatement("INSERT INTO mir_datatype (datatype_id, name, pattern, definition, date_creation, date_modif) VALUES (?, ?, ?, ?, NOW(), NOW())");
            try
            {
...
*/
            ListIterator it = getResources().listIterator();
            int cpt = 0;
            PreparedStatement stmt06 = pool.getPreparedStatement("INSERT INTO mir_resource (resource_id, url_element_prefix, url_element_suffix, url_resource, info, institution, location, obsolete, ptr_datatype) VALUES (?, ?, ?, ?, ?, ?, ?, '0', ?)");
            while (it.hasNext())
            {
                // generation of a new ID for the resource
                Resource temp = (Resource) it.next();
...
*/
            PreparedStatement stmt07 = pool.getPreparedStatement("INSERT INTO mir_doc (uri, uri_type, ptr_type, ptr_datatype, ptr_resource) VALUES (?, 'MIRIAM', 'data', ?, NULL)");
            for (int i=0; i<getDocumentationIDs().size(); ++i)
            {
                try
                {
                    stmt07.setString(1, getDocumentationID(i));
                    stmt07.setString(2, index);
...
*/
	...
	public void retrieveData(String poolName, String index)
	{
		...
		// search for the date of creation
...
/* OLD VERSION: without prepared statements
 * 
			sql = "UPDATE mir_datatype SET name='" + getName() + "', pattern='" + getRegexp() + "', definition='" + getDefinition() + "', date_modif=NOW() WHERE (datatype_id='" + getId() + "')";
			resultStatus = pool.requestUpdate(pool.getStatement(), sql);
			logger.info("Result of the update query #1 (edit data-type, resource): " + resultStatus);
			resultGlobal= resultGlobal * resultStatus;   // if the result is "1" (means succes), the 'resultGlobal' will stay at the value "1" (means everything is OK)
*/
            
            PreparedStatement stmt01 = pool.getPreparedStatement("UPDATE mir_datatype SET name=?, definition=?, pattern=?, date_modif=NOW() WHERE (datatype_id=?)");
            try
            {
                stmt01.setString(1, getName());
                stmt01.setString(2, getDefinition());
                stmt01.setString(3, getRegexp());
                stmt01.setString(4, getId());
                logger.debug("SQL query: " + stmt01.toString());
                resultStatus = stmt01.executeUpdate();
                stmt01.close();
            }
...
*/
            Iterator itr = oldResources.iterator();
            int cpt = 0;
            PreparedStatement stmt08 = pool.getPreparedStatement("UPDATE mir_resource SET url_element_prefix=?, url_element_suffix=?, url_resource=?, info=?, institution=?, location=?, obsolete='0' WHERE (resource_id=?)");
            PreparedStatement stmt08obs = pool.getPreparedStatement("UPDATE mir_resource SET url_element_prefix=?, url_element_suffix=?, url_resource=?, info=?, institution=?, location=?, obsolete='1' WHERE (resource_id=?)");
            while (itr.hasNext())
            {
                Resource r = (Resource) itr.next();
                try
...
*/
            ...
            cpt = 0;
            // a new resource can't be 'obsolete'
            PreparedStatement stmt09 = pool.getPreparedStatement("INSERT INTO mir_resource (resource_id, url_element_prefix, url_element_suffix, url_resource, info, institution, location, obsolete, ptr_datatype) VALUES (?, ?, ?, ?, ?, ?, ?, '0', '" + getId() + "')");
            while (it3.hasNext())
            {
                Resource temp = (Resource) it3.next();
                try
...
*/
            PreparedStatement stmt12 = pool.getPreparedStatement("INSERT INTO mir_doc (uri, uri_type, ptr_type, ptr_datatype, ptr_resource) VALUES (?, 'MIRIAM', 'data', '" + getId() + "', NULL)");
            for (int i=0; i<getDocumentationIDs().size(); ++i)
            {
                try
                {
...
*/
        ...
        ArrayList deletedResources = new ArrayList();   // removed resources (to put as "deprecated")
        ArrayList newResources = new ArrayList();   // new resources
        ArrayList oldResources = new ArrayList();   // old undeleted resources
        ...
        {
            Resource obj = (Resource) it2.next();
            ListIterator temp = newList.listIterator();
            Resource tmp = null;
            boolean find = false;
            while (temp.hasNext())
            {
    ...
    public String diff(DataTypeHybernate data)
    {
        ...
        for (int i=0; i<this.getResources().size(); ++i)
        {
            ...
            ListIterator it1 = data.getResources().listIterator();
            Resource tmp1 = null;
            while (it1.hasNext())
            {
        ...
        {
            boolean find2 = false;
            ListIterator it2 = this.getResources().listIterator();
            Resource tmp2 = null;
